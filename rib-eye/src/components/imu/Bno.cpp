...
void Bno::update() {
    ...
    if (now - _lrt >= _interval_ms) {

        _lrt += _interval_ms;

        eData = _bno.getVector(
            Adafruit_BNO055::VECTOR_EULER
        );

        qData = _bno.getQuat();

        aData = _bno.getVector(
            Adafruit_BNO055::VECTOR_LINEARACCEL
        );
    }
}

void Bno::getCalibration(
    uint8_t* sys,
    uint8_t* gyro,
    uint8_t* accel,
    uint8_t* mag
...
imu::Quaternion Bno::getQuat() const {
    return qData;
}

imu::Vector<3> Bno::getLinealAcc() const {
    return aData;
}
